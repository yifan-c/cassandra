...
final class BatchUpdatesCollector implements UpdatesCollector
{
    ...
    BatchUpdatesCollector(Map<TableId, RegularAndStaticColumns> updatedColumns, Map<TableId, Map<ByteBuffer, Integer>> perPartitionKeyCounts)
    {
        super();
    ...
    public PartitionUpdate.Builder getPartitionUpdateBuilder(TableMetadata metadata, DecoratedKey dk, ConsistencyLevel consistency)
    {
        ...
        if (upd == null)
        {
            ...
            assert columns != null;
            upd = new PartitionUpdate.Builder(metadata, dk, columns, perPartitionKeyCounts.get(metadata.id).get(dk.getKey()));
            mut.add(upd);
        }
        return upd;
    }
